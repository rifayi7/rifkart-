...
const colors = [
  { name: "Orange", bg: "bg-orange-400", ring: "ring-orange-500", count: 2 },
  { name: "Pink", bg: "bg-pink-300", ring: "ring-pink-400", count: 3 },
  { name: "Green", bg: "bg-green-300", ring: "ring-green-400", count: 1 },
  { name: "Blue", bg: "bg-blue-300", ring: "ring-blue-400", count: 2 },
  { name: "Brown", bg: "bg-yellow-900", ring: "ring-yellow-800", count: 5 },
  { name: "White", bg: "bg-gray-200", ring: "ring-gray-300", count: 1 },
  { name: "Yellow", bg: "bg-yellow-300", ring: "ring-yellow-400", count: 1 },
  { name: "Purple", bg: "bg-purple-300", ring: "ring-purple-400", count: 1 },
];

export default function FilterByColour({ colorFilter }: any) {
  return (
    <div className="flex flex-col gap-3 text-gray-600 font-light">
      <h1 className="font-bold text-black">Filter by colour</h1>
      {colors.map((color, idx) => (
        ...
        >
          <div
            onClick={() => colorFilter(color.name)}
            className="flex items-center gap-3 group-hover:text-amber-300"
          >
            <div
              className={`w-4 h-4 rounded-full ring-2 p-[2px] ring-gray-400 group-hover:ring-gray-600`}
            >
              <div className={`w-full h-full rounded-full ${color.bg}`}></div>
            </div>
            <span>{color.name}</span>
